...
// we can actually make use of gotos without ie being very bad form.
//
...
Dungeon_T *load_dungeon(const char *path, bool stairs) {
    ...
    // and warn if it's not that it won't be a good dungeon. Ugly cast to grab than more one byte is needed. Need to do
    // endian conversion.
    ...
    // Get how many upwards stairs there are and warn if there is none and stairs bool is false.
    // Ugly cast to grab than more one byte is needed. Need to do endian conversion.
    if (ntohs(*(uint16_t *) &buffer[p]) == 0 && !stairs) {
        fprintf(stderr,
                "Dungeon does not have upwards staircases and will be mostly unplayable! Consider using a random dungeon or using the stairs switch!\n");
    }
    r = ntohs(*(uint16_t *) &buffer[p]); // convert back to host endian order for next step.
    p += sizeof(uint16_t);
    ...
    // Get how many upwards stairs there are and warn if there is none.
    // Ugly cast to grab than more one byte is needed. Need to do endian conversion.
    if (ntohs(*(uint16_t *) &buffer[p]) == 0 && !stairs) {
        fprintf(stderr,
                "Dungeon does not have downwards staircases and will be mostly unplayable! Consider using a random dungeon or using the stairs switch!\n");
    }
...
void save_dungeon(Dungeon_T *d, const char *path) {
    ...
    // Insert file marker
    memcpy(&buffer[p], marker, strlen(marker)); // NOLINT
    p += (int) strlen(marker);
