...
static void read_arguments(int argc, char *argv[], Arguments_T *a) {
    int i;
    ...
    while (i < argc) {
        ...
        if (strcmp(argv[i], SAVE_LONG) == 0 || strcmp(argv[i], SAVE_SHORT) == 0) {
            ...
            // Check if there is a path specified
            if (i < argc && !is_argument_string(argv[i])) {
                a->save_dungeon_path = safe_malloc(strlen(argv[i]) + sizeof("\0")); // allocate space for null terminator
...
void init_program(int argc, char *argv[], Program_T *p) {
