...
static int32_t distance_path_cmp(const void *key, const void *with)
{
  return ((distance_path_t *)key)->cost - ((distance_path_t *)with)->cost;
}
...
void generate_distance_map_tunnel(dungeon_t *d)
{
  ...
  heap_init(&h, distance_path_cmp, NULL);
  ...
  while ((p = heap_remove_min(&h)))
  {
    ...
    int weight = 1 + (hardnesspair(p->pos) / 85);
...
void generate_distance_map_non_tunnel(dungeon_t *d)
{
  static distance_path_t path[DUNGEON_Y][DUNGEON_X], *p;
  static uint32_t initialized = 0;
  heap_t h;
...
void init_nontunnel_distance_map(dungeon_t *d)
{
  uint32_t x, y;
  ...
  for (y = 0; y < DUNGEON_Y; y++)
  {
    for (x = 0; x < DUNGEON_X; x++)
    {
      d -> nontunnel_distance_map[y][x] = -1;
    }
  }
}
...
void print_tunnel_distance_map(dungeon_t *d)
{
  ...
  for (y = 0; y < DUNGEON_Y; y++)
  {
    for (x = 0; x < DUNGEON_X; x++)
    {
      ...
      {
        printf("%d", d->tunnel_distance_map[y][x] % 10);
      }
    }
    printf("\n");
  }
...
int main(int argc, char *argv[])
{
  ...
  render_dungeon(&d);
  generate_distance_map_tunnel(&d);
  init_nontunnel_distance_map(&d);
  generate_distance_map_non_tunnel(&d);
