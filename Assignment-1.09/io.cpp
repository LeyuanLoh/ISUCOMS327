...
//Lee's
void object_to_string(object *obj, char *s, int size)
{
  if(obj){
  snprintf(s,size,"%s (sp: %d, dmg: %d+%dd%d)",obj->get_name(), 
                                               obj->get_speed(), 
                                               obj->get_damage_base(),
                                               obj->get_damage_number(),
                                               obj->get_damage_sides()); 
}
else{
  snprintf(s,size, "%s"," ");
}
...
static void io_list_equipment(dungeon *d)
{
  ...
  for (i = 0; i < num_eq_slots; i++) {
    sprintf(s, "[%s]", eq_slot_type[i]);
    object_to_string(d->PC->equipment[i], t, 60);
...
//Lee's
static void io_drop_inventory(dungeon *d)
{
  uint32_t i;
  char s[60];
  int key;
  ...
  mvprintw(17, 9, " %-60s ", "");
  mvprintw(18, 9, " %-60s ", "Which item to drop?(0-9)(Esc to cancel)");
...
//Lee's
static void io_expunge_inventory(dungeon *d)
{
  uint32_t i;
  char s[60];
  int key;
  ...
  mvprintw(17, 9, " %-60s ", "");
  mvprintw(18, 9, " %-60s ", "Which item to expunge?(0-9)(Esc to cancel)");
  ...
  do{
    ...
    if(key>='0'&& key <= '9'){
      valid = d->PC->expunge_item_on_slot(d,key-'0');
...
uint32_t io_display_item_description_on_slot(dungeon *d, uint32_t slot)
{
  uint32_t i, l;
  uint32_t n;
  //empty stirng to remove background
  char empty[80];
  ...
  if(!d->PC->inventory[slot]){
    return 0;
  }
  else{
    i = 0;
    n= 0; 
    l = strlen(d->PC->inventory[slot]->get_description());
    for (i = 0; i < l; i++) {
      if (d->PC->inventory[slot]->get_description()[i] == '\n') {
        n++;
      }
    }
    ...
    mvprintw(3,0,d->PC->inventory[slot]->get_description());
...
static void io_inspect_item(dungeon *d)
{
  ...
  mvprintw(17, 9, " %-60s ", "");
  mvprintw(18, 9, " %-60s ", "Which item to inspect?(0-9)(Esc to cancel)");
  ...
  do{
    ...
    if(key>='0'&& key <= '9'){
      valid = io_display_item_description_on_slot(d, key-'0');
...
void io_look_on_monster(dungeon *d)
{
  pair_t dest;
  int c;
  fd_set readfs;
  ...
  mvprintw(0, 0,
           "Choose a monster to look on.  't' to select and ESC to cancel");
  ...
  * not otherwise used.                                                 */
  ...
  if( c== 't'){
    if (!charpair(dest) || charpair(dest) == d->PC) {
      io_queue_message("There is no monster on the place selected");
    } 
    else {
      int i;
      for (i =0; i<=19; i++){
      ...
      character *monster_selected = d->character_map[dest[dim_y]][dest[dim_x]];

      mvprintw(3,0, dynamic_cast<npc *>(monster_selected)->description);
      
      mvprintw(18, 9, " %-60s ", "Press any key to contiunue");
